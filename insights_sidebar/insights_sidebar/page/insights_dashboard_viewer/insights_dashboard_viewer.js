...
function _load_dashboard(page, $container, dashboardName, label) {
    ...
    // Frappe Insights usually serves dashboards under the Desk route: /app/insights/dashboard/<name>
    const dashboardUrl = /^(https?:\/\/|\/)/.test(dashboardName)
        ? dashboardName
        : `/app/insights/dashboard/${encodeURIComponent(dashboardName)}`;
